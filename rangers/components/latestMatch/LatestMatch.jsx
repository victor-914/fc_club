...
function LatestMatch({ item }) {
  const [latestMatch, setLatestMatch] = useState(null);
  console.log(
    "🚀 ~ file: LatestMatch.jsx:10 ~ LatestMatch ~ latestMatch:",
    latestMatch
  );
  ...
  useEffect(() => {
    setLatestMatch(item?.data?.[0]);

    return () => {};
  }, []);

  const loaderHome = () => {
    return latestMatch?.attributes?.home_img?.data?.attributes?.url;
  };

  const loaderAway = () => {
    return latestMatch?.attributes?.away_img?.data?.attributes?.url;
  };
        ...
        <main className="fixtureContainer">
          <div className="home fixture">
            <div className="name homeName">
              {latestMatch?.attributes?.homeTeamTitle}
            </div>
            ...
            <div className="nums">{latestMatch?.attributes?.home_score}</div>
          </div>
          <aside className="vs">-</aside>
          <div className="away fixture">
            <div className="nums">{latestMatch?.attributes?.away_score}</div>
            <div className="logoCont">
              <div className=" logo awayLogo">
                <Image
                  loader={loaderAway}
                  // src={fixture?.attributes?.away_logo?.data?.attributes?.url}
                  className="img"
                  layout="fill"
                  src={rangers}
                  placeholder="blur"
            ...
            <div className="name awayName">
              {latestMatch?.attributes?.awayTeamTitle}
            </div>
          </div>
        </main>
